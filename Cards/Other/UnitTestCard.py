...
class RankTest(unittest.TestCase):
  ...
  def testRankName(self): # unit test for rank names, 'ace', 'two', 'three',...
    ...
    for i in RANK_NAMES:    
        myCard = Card(RANK_NAMES.index(i) + 2, 's')  # creates the value of the card 
        self.assertEqual(myCard.rankName(), i)       # by adding 2 to the index of RANK_NAMES
...
class SuitTest(unittest.TestCase):
  ...
  def testSuitName(self): # unit test for suit names, 'clubs', 'diamonds',...
    ...
    for i in range(2,15):   # creates all the cards with all the suits
        for suit in 'cdhs':
            myCard = Card(i, suit)
            self.assertEqual(myCard.suitName(),SUIT_NAMES[SUITS.index(suit)])
